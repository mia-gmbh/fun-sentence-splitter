...
- `poetry lock && poetry install`, dann das Sprachmodell ziehen: `poetry run python -m spacy download de_core_news_sm`.
- `ruff check . && mypy . && pytest` müssen grün sein; nötige Fixes gehören in diesen Commit.
...
*Getestet wird sm/md/lg. Auf ein größeres Modell nur wechseln, wenn neue spaCy-Version + größeres Modell ≥10% besser sind; sonst diskutieren (größere Modelle sind deutlich langsamer). Läuft im Poetry-Venv (Skript nutzt `poetry run`, nichts global).*
